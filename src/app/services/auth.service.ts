...
const registerUser = async (
  ...
  profile_data: ICreateProfile
) => {
  const email = user_data.email.toLowerCase();
  ...
  try {
    const { user } = await Repository.transaction(async (trx) => {
      ...
      // Return the results
      return { user, profile, authentication, updated_homedata };
    });
...
const resendCode = async (user_id: string) => {
  ...
  if (
    latest_auth &&
    !isExpired(latest_auth.expire_time) &&
    latest_auth.is_success === false
  ) {
